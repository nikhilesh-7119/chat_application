...
class _PostScreenState extends State<PostScreen> {
  ...
  Widget build(BuildContext context) {
    // Page layout: Upload container at top; below it the recent posts list (timestamp)
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            // Create New Post
            UploadPostContainer(
              onSubmitted: (title, category, description, authorId) async {
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  // Example "All Categories" action opens a picker and then reloads category list
                  IconButton(
                    icon: const Icon(Icons.filter_list),
                    onPressed: () async {
                      final selected = await _pickCategory(context);
                      if (selected == null || selected.isEmpty) {
                        // Back to timestamp feed
                        await controller.getPostsByTimestamp(reset: true);
                      } else {
                        await controller.getPostsByCategory(category: selected, reset: true); // [3]
                      }
                    },
                  ),
                ],
            ...
            Expanded(
              child: Obx(() {
                // If a category is active (controller maintains current category), show category list
                ...
                return RefreshIndicator(
                  onRefresh: () async {
                    if (showingCategory) {
                      // Reload current category from start
                      // Note: controller tracks current category internally
                      await controller.getPostsByCategory(
                        category: (controller
                                .getPostByCategory.isNotEmpty
                            ? controller.getPostByCategory.first.category ?? ''
                            : ''),
                        reset: true,
                      ); // [3][18]
                    } else {
                      await controller.getPostsByTimestamp(reset: true); // [17][18]
                    }
                  },
                      ...
                      SliverList(
                        delegate: SliverChildBuilderDelegate(
                          (context, index) {
                            ...
                            return PostCard(post: post); // card widget
                          },
  ...
  Future<String?> _pickCategory(BuildContext context) async {
    // Simple bottom sheet for picking category (replace with actual categories)
